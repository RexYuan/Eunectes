...
We came up with an alternative heuristic for a general purpose workflow. Since MIs are bounded by the minimum of the pair's respective Shannon entropies, we add additional edges according to a fraction of this upper bounds.

For the measure step, we followed the examples provided in the tool's repository. Gaussian noises are added to the selected marginals. Half of the privacy budget are spent on all 1-way marginals and the other half on the selected cliques. These marginals are then fed to the tool to fit the MRF. By \cite{mckenna2021winning}, this procedure satisfies $(\alpha,\frac{\alpha}{2 \sigma^2})$-RDP for all $\alpha \geq 1$.
